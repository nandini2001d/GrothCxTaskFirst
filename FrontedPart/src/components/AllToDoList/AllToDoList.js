...
export default function AllToDoList() {
  ...
  const getalldata = async () => {
    await fetch(
      `http://localhost:3456/todolists/email/${localStorage.getItem("email")}`
    )
      .then((value) => {
  ...
  const seachbytitle = async () => {
    await fetch(`http://localhost:3456/todolists/title/${title}`)
      .then((value) => {
        value.json().then((result) => {
          setDataAll(result);
        });
  ...
  const putmethod = async (icode, id) => {
    await fetch(
      `http://localhost:3456/todolists/status/${id}?id=${id}&status=${icode}`,
      {
        method: "PUT",
      }
    )
  ...
  const filterbystatus = async (valueset) => {
    await fetch(
      `http://localhost:3456/todolists/status/${valueset}/${localStorage.getItem(
        "email"
      )}`
    )
      .then((value) => {
